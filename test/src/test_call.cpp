...
TEST(call, call_operator)
{
...

    {
        ...
        uint32_t j = 2U;
        EXPECT_TRUE(function.expect_calls().with(j).check());
    }
...
{
    {
        ...
        EXPECT_FALSE(function.expect_calls().with(2,3).with(4,5).check());
    }

    {
        stub::call<void(uint32_t,uint32_t)> function;

        EXPECT_FALSE(function.expect_calls().with(2,3).repeat(10).check());
    }

    {
        ...
        EXPECT_FALSE(function.expect_calls().with(2,3).check());
    }
}
...
TEST(call, expect_calls_with_ignore)
{
    ...
    EXPECT_TRUE(function.expect_calls().ignore(4).with(2,6).check());
...
TEST(call, use_check)
{
    ...
    EXPECT_TRUE(function.expect_calls().with(1).with(2).check());
    EXPECT_FALSE(function.expect_calls().with(1).check());
}
...
TEST(call, expect_predicate_custom_type)
{
    ...
    EXPECT_TRUE(function.expect_calls(p).with(cup{2.3}).with(cup{4.5}).check());
}
...
TEST(call, expect_predicate_pair)
{
    ...
    EXPECT_TRUE(function.expect_calls(p).with(element(10,3)).check());

    function(element(20,3));

    // We have called more than once
    EXPECT_FALSE(function.expect_calls(p).with(element(10,3)).check());
...
TEST(call, predicate_argument_order)
{
    ...
    EXPECT_TRUE(function.expect_calls(p).with(10U).check());
}
